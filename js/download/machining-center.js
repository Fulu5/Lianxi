jQuery(function($){
	...
	var thisData = function(curr){			    
	    ...
	    for(var i = (curr*nums - nums)+1; i <= last; i++){	
	        var partArray = Enumerable.from(machining).where(function(x){return x.id == i;}).toArray();	
	...
	laypage({
	    ...
	    pages: pages,
	    jump: function(obj){
	        document.getElementById('machining_list').innerHTML = thisData(obj.curr);	
	        //阻止子元素触发父元素的点击事件
