jQuery(function($){
	...
	var thisData = function(curr){			    
	    ...
	    for(var i = (curr*nums - nums)+1; i <= last; i++){	
	        var partArray = Enumerable.from(accessory).where(function(x){return x.id == i;}).toArray();	
			var img = partArray[0].img;
			var title = partArray[0].title;
			var pdf_addr = partArray[0].pdf_addr;
			var pdf_title = partArray[0].pdf_title;
	        /*str += '<li><a href='+pdf_addr+' target="_blank">';
	        str += '<img class="item-img" src='+img+'></a>';
	        str += '<p class="little-title">'+title+'</p>';
	...
	laypage({
	    ...
	    pages: pages,
	    jump: function(obj){
	        document.getElementById('accessory_list').innerHTML = thisData(obj.curr);	
	        //阻止子元素触发父元素的点击事件
